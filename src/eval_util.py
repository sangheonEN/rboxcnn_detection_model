...
filepath = 'D:/my/KITECH/ship_detection/haekwang/1st_data/ship_detection/1st_exam/simplified_rbox_cnn-master/simplified_rbox_cnn-master'
...
def evaluate_detection_results_pascal_voc(result_lists,
    ...
    Args:
      ...
      box_type: string for box type of ['box', 'rbox']
    ...
    evaluator = object_detection_evaluation.ObjectDetectionEvaluation(
        num_classes, matching_iou_threshold=iou_thres, box_type=box_type,
    ...
    for idx, image_id in enumerate(image_ids):
        ...
        if is_save_detection_results:
            evaluator.add_single_image_and_info(image=result_lists['original_image'][idx],
                                                sensor=result_lists['sensor'][idx],
                                                filename=result_lists['filename'][idx])
    ...
    if is_save_detection_results:
        evaluator.save_detection_results(detection_results_name)
...
def visualize_detection_results(result_dict,
    ...
    image = np.squeeze(result_dict['original_image'], axis=0).copy()
...
def run_checkpoint_once(tensor_dict,
    ...
    Args:
      ...
      is_rbbox: Whether or not the rbbox type
...
def all_checkpoint_run(tensor_dict,
                       update_op,
                       summary_dir,
                       aggregated_result_processor=None,
                       batch_processor=None,
                       ...
                       metric_names_to_values=None,
                       keys_to_exclude_from_results=()):
    """evaluates desired tensors using checkpoint_dirs or restorer for all checkpoints.

    This function loads all checkpoints and evaluates a desired
    set of tensors (provided by tensor_dict) and hands the resulting numpy
    arrays to a function result_processor which can be used to further
    process/save/visualize the results.
    ...
    Args:
      ...
      variables_to_restore: None, or a dictionary mapping variable names found in
        ...
        generated by creating a tf.train.ExponentialMovingAverage object and
        calling its variables_to_restore() method. Not used if restore_fn is set.
      restorer: a function that takes a tf.Session object and correctly restores
        all necessary variables from the correct checkpoint file.
      num_batches: the number of batches to use for evaluation.
      master: the location of the Tensorflow session.
      save_graph: whether or not the Tensorflow graph is saved as a pbtxt file.
    ...
    while not restorer.all_checked():
        run_checkpoint_once(tensor_dict, update_op, summary_dir,
                            aggregated_result_processor,
                            batch_processor, checkpoint_dirs,
                            variables_to_restore, restorer, num_batches, master,
                            save_graph, save_graph_dir, metric_names_to_values,
                            keys_to_exclude_from_results)
