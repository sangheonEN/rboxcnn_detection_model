...
def train(create_tensor_dict_fn, create_model_fn, train_config, input_config, master, task,
    ...
    with tf.Graph().as_default():
        ...
        """
        train_tensor : optimizer
        train_dir : tf.saver ckpt save dir
        log_every_n_steps : model save step
        master : ??
        is_chief : ??
        session_config : tf.ConfigProto ??
        startup_delay_steps : ?? Not find config file 
        init_fn : back bone load resnet101 ckpt file -> save.restore(resnet101.ckpt_file)  
        summary_op : tensorboard summary op
        train_config.num_steps : total train num
        save_summaries_secs :
        save_interval_secs : 
        sync_optimizer : 
        saver : 
