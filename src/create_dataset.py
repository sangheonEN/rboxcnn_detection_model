    ...
    "features": [
        {
            ...
            "properties": {
                "bounds_imcoords": "2901.65,41.22,2931.06,32.16,2957.06,116.56,2927.64,125.62",
                "edited_by": "Ilwon Lee @ SI Analytics",
                "feature_id": [
                    "3598228ec8be93ff57c2d81f35a223e41ff01f97799639e3b3fc09be526185f2"
                ],
                "image_id": "0000.png",
...
def cvt_coords_to_polys(coords):
    ...
    ex) "2901.65,41.22,2931.06,32.16,2957.06,116.56,2927.64,125.62"
...
def create_tfrecords(src_dir, dst_path, patch_size=1024, patch_overlay=384, object_fraction_thresh=0.7,
    ...
    # 중복 무시하고 모든 list num을 보니까 image수 보다 작더라. 그럼 label 정보에 해당 image 정보가 없다는 말인데? 모든 image에 label이 있는게 아니구나.
...
if __name__ == '__main__':
    """
    TFRecords 파일은 patch로 분할하여 이미지 파일을 읽음. 데이터 직렬화 파일 포맷으로 많은 양의 데이터를 빠르게 읽을 수 있음.
    patch_size : 분할 patch size
    patch_overlay : 겹침 patch size (주변 patch들과 겹치는 부분을 만들어 ship 짤림 현상 방지)
    object_fraction_thresh : 객체가 짤린 경우 허용 비율
    is_include_only_pos : 객체가 있는 patch만 포함한다.
    """
    ...
    # 가변 변수로 args 전달 [src_dir, dst_path, patch_size, patch_overlay, object_fraction_thresh, is_include_only_pos]
